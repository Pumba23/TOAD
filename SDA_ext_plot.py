...
def plot_SDA_new(m_results_years, names_factors, num_factors, plot_period, input0, reg_to_reg_type, sda_type,
    ...
    if reg_to_reg_type == 'dom1':
        legend_name = "Extended SDA - Demand World Approach"
    if reg_to_reg_type == 'dom2':
        legend_name = "Extended SDA - Domestic Demand Approach"
    if reg_to_reg_type == 'cb':
        legend_name = "Extended SDA - Consumption-based Approach"
    if reg_to_reg_type == 'pb':
        legend_name = "Extended SDA - Production-based Approach"
    if reg_to_reg_type == 'saldo':
        legend_name = "Extended SDA - Balanced Approach"
    if reg_to_reg_type == 'from1to2':
        legend_name = "Extended SDA - Imports"
    if reg_to_reg_type == 'from2to1':
        legend_name = "Extended SDA - Exports"
    ...
    print(f"Length of positionen: {len(positionen)}")
    print(f"Length of summe_pro_jahr: {len(summe_pro_jahr)}")
    ...
    legend = plt.legend(loc='lower left', bbox_to_anchor=(0.02, 0.03), ncol=2, fontsize=15)

    if reg_to_reg_type == 'cb' and dekomp_type == 'yxy':
        legend = plt.legend(loc='upper center', bbox_to_anchor=(0.19, 0.99), ncol=2, fontsize=13)
        y_max = y_max + 50
    ...
    if reg_to_reg_type == 'from1to2' and dekomp_type == 'yxy':
        legend = plt.legend(loc='upper center', bbox_to_anchor=(0.19, 0.99), ncol=2, fontsize=13)
        y_max = y_max + 35
    if reg_to_reg_type == 'from2to1' and dekomp_type == 'yxy':
        legend = plt.legend(loc='upper center', bbox_to_anchor=(0.19, 0.99), ncol=2, fontsize=13)
